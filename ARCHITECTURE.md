...
- **Agentic analyst** (`agentic_analyst=True`). Replaces the single-shot analyst with a tiered pair: a cheap legwork model (local Qwen when `VLLM_URL` is set, Haiku otherwise) runs a bounded tool loop over the same retrieval tools (`graph/agent_loop.py`, the critic's turn / wall-clock containment pattern), then Sonnet writes and cites over only the gathered evidence, so the write step matches the single-shot analyst by construction. Legwork failure or empty evidence falls back to the single-shot analyst, which is also the A/B baseline.
